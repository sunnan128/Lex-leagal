...
3. **RRF 融合**：语义检索与关键词检索各取 top-20，通过 Reciprocal Rank Fusion（`RRF_K=20`，基于排序位置融合）合成候选池，不受语义/关键词分数量级差异影响，取 top-20 候选
4. **重排序精排**：bge-reranker-base CrossEncoder 对 top-20 候选逐对打分，精排取 top-5 给 LLM
...
基于 **30 条 QA 对、10 个法律领域**的评估数据集，对比 Baseline（无 Rerank）和 Rerank（CrossEncoder 精排）两种模式的量化效果。
